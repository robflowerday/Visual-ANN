...
# Instantiate a network class object with the number of neurons
# in its input layer and its learning rate.
network = Network(num_input_neurons=28*28, learning_rate=0.3)

# Add hidden layers, and an output layer to the network, stating
# the number of neurons, activation function to be used at each
# layer of the network and the differential of the activation
# function.
network.add_layer(
    num_neurons=2,
...
# Tell the network to initialise parameter vectors and lists.
# This must be done after all the layers in the network have
# been defined.
network.initialise_parameter_arrays()

# Define the data that will train the network.
...
def train_on_selection_of_mnist_samples(network, iterations, num_samples):
    """ Train the network using forward and back propagation.

        The calculation of this function is all performed in the
        network.train() method, the rest of the function deals with
        looping and logging. """
    for i in range(iterations):
        ...
        end_time = time.time()
        print(f"{i}: Cost: {network.cost}   - took {end_time - start_time} seconds to run.")
